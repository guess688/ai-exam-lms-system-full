...
public class FileServiceImpl implements IFileService {
    private static final long COURSE_FILE_MAX_SIZE = 200L * 1024 * 1024;
    ...
    public Result<String> uploadFile(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new ServiceRuntimeException("上传文件不能为空");
        }
        if (file.getSize() > COURSE_FILE_MAX_SIZE) {
            throw new ServiceRuntimeException("课程文件不能超过200MB");
        }
        String url = fileService.upload(file);
        if (StringUtils.isBlank(url)) {
            throw new ServiceRuntimeException("课程文件上传失败，url地址没有返回");
        }
        return Result.success("课程文件上传成功", url);
    }
}
