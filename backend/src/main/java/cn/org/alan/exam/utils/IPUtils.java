...
public class IPUtils {
    ...
    private static final String DB_RESOURCE = "ipdata/ip2region.xdb";

    private static final ThreadLocal<Searcher> searcherThreadLocal = ThreadLocal.withInitial(() -> {
        try {
            InputStream inputStream = IPUtils.class.getClassLoader().getResourceAsStream(DB_RESOURCE);
            if (inputStream != null) {
                try (InputStream stream = inputStream; ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
                    ...
                    return Searcher.newWithBuffer(outputStream.toByteArray());
                }
            }
