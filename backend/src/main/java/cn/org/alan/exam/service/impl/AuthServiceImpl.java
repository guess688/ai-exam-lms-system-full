...
public class AuthServiceImpl implements IAuthService {
    ...
    public Result<String> login(HttpServletRequest request, LoginForm loginForm) {
        // 先判断用户是否通过校验
        boolean captchaRequiredForLogin = false;
        if (captchaRequiredForLogin && StringUtils.isBlank(stringRedisTemplate.opsForValue().get("isVerifyCode" + request.getSession().getId())) && captchaEnabled) {
